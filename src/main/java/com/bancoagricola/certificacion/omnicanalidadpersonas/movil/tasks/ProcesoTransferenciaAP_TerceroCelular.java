...
public class ProcesoTransferenciaAP_TerceroCelular implements Task {
    private final List<Ahorros> datos;

    public ProcesoTransferenciaAP_TerceroCelular(List<Ahorros> datos) {
        this.datos = datos;
    }

    @Step("{0} realiza el proceso de transferencia a un numero de celular asociado a una cuenta de tercero")
    @Override
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                ...
                ClickEn.elElemento(UiGenerico.elementoQueContengaElTexto(TXTCELULAR)),
                IngresarEn.elCampo(TXTCELULAR).elValor(aho.getCelular()),
    ...
    public static ProcesoTransferenciaAP_TerceroCelular para(List<Ahorros> datos) {
        return Instrumented.instanceOf(ProcesoTransferenciaAP_TerceroCelular.class).withProperties(datos);
    }
}
